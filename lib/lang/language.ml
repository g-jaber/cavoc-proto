(** The [Language] module contains the various signatures to represent the core
...
module type COMP = sig
  ...
  (** Evaluation of programs may produce:
      ...
      The first two cases are represented using the
      {!constructor: EvalMonad.PropStop} constructor. The other cases are
      returned wrapping the {!opconf} in the {!constructor: EvalMonad.Continue}
      constructor. *)
  ...
  val get_typed_opconf :
    ...
    string ->
    Lexing.lexbuf ->
    opconf * typ * Namectx.t
  (** [opponent_signature], when provided, is the signature of names the expression may use
      but does not define, as in {!get_typed_ienv}: they become Opponent names
      of the program. *)
  ...
  (** [get_typed_ienv] evaluates a module (a list of computation) into a list of
      value declarations. It returns the generated interactive environment along
      with the store generated by its evaluation, and two typing contexts.
      ...
      does not implement: they become Opponent names, and so appear in the
      returned Opponent name context. Omitting it is the standalone case, and is
      equivalent to passing an empty signature. *)

  val get_typed_namectx : Lexing.lexbuf -> Namectx.t
  (** [get_typed_namectx] returns the name context a signature declares, with no
      implementation behind it: the Proponent context of an arena the two
      participants of which are both to be synthesized. *)
end

(** This signature contains the definition of normal forms *)
module type NF = sig
  type ('value, 'ectx, 'fname, 'cname) nf_term [@@deriving to_yojson]
  (** Normal forms are parametrized w.r.t. the type of values, evaluation
      contexts, function names, and continuation names. *)
  ...
  val string_of_nf_term :
    ...
    ('value, 'ectx, 'fname, 'cname) nf_term ->
    string
  (** The first argument is a string inserted between the negative part of the
      normal form and the values filling the positive parts *)

  val is_error : ('value, 'ectx, 'fname, 'cname) nf_term -> bool

  val case_nf_term :
    on_call:('fname -> 'value -> 'ectx -> 'a) ->
    on_return:('cname -> 'value -> 'a) ->
    on_raise:('cname -> 'value -> 'a) ->
    on_error:('cname -> 'a) ->
    ('value, 'ectx, 'fname, 'cname) nf_term ->
    'a
  (** The case analysis over the four shapes of a normal form. *)
...
(** [WITHAVAL_INOUT] is used for operational languages with normal forms that do
    not have necessarily a head name. This is for example the case of terms that
    are not named, so that values are normal forms. *)
...
(** [WITHAVAL_NEG] is used for operational languages with normal forms that have
    necessarily a head name. This is for example the case of languages with
    named terms, as in the λμ-calculus. *)
