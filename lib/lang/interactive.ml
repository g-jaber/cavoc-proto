module type LANG = sig
  ...
  val eval :
    ...
    ((abstract_normal_form * IEnv.Renaming.Namectx.t * Storectx.t) * IEnv.t * store)
    EvalMonad.m
  ...
  val generate_a_nf :
    ...
    (abstract_normal_form * IEnv.Renaming.Namectx.t * IEnv.Renaming.Namectx.t) BranchMonad.m
  ...
  val concretize_a_nf :
    store -> IEnv.t -> abstract_normal_form * IEnv.Renaming.Namectx.t -> opconf * IEnv.t
end

module type LANG_WITH_INIT = sig
  include LANG

  val get_typed_opconf : string -> Lexing.lexbuf -> opconf * IEnv.Renaming.Namectx.t
  ...
  val get_typed_ienv :
    Lexing.lexbuf -> Lexing.lexbuf -> IEnv.t * store * IEnv.Renaming.Namectx.t * IEnv.Renaming.Namectx.t
end
  ...
  let pp_a_nf ~pp_dir fmt (a_nf_term, store) =
    ...
    let pp_a_nf_term =
      OpLang.Nf.pp_nf_term ~pp_dir OpLang.AVal.pp_abstract_val pp_ectx
        OpLang.IEnv.Renaming.Namectx.Names.pp_name OpLang.IEnv.Renaming.Namectx.Names.pp_name in
