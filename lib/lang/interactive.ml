module type LANG = sig
  ...
  val generate_a_nf :
    Store.store_ctx -> name_ctx -> (abstract_normal_form * name_ctx * name_ctx) M.m

  (* The typing judgment of an abstracted normal form Γ_P;Γ_O ⊢ A ▷ Δ
     produces the interactive name contexts (Δ,Γ'_P) of fresh names introduced by A.
...
module Make (OpLang : Language.WITHAVAL_NEG) : LANG = struct
  ...
  let string_of_nf (nf_term, _) =
    string_of_nf_term "" string_of_value nf_term
  ...
  let abstracting_normal_form (nf_term, store) namectxO storectx_discl =
    ...
    if OpLang.Nf.is_error a_nf_term then None else
