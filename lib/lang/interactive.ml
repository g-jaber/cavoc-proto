module type LANG = sig
  ...
  (* fold_free_names_of_a_nf f acc A folds f over the free names of A:
     its subject name, then the free names of its abstract values.
     The store part of A is not traversed (disclosure through the store
     is not yet handled, cf. renaming_a_nf). *)
  ...
  (* map_free_names_of_a_nf f A renames the free names of A along f: its
     subject name and the free names of its abstract values. The store
     part of A is not traversed, as for fold_free_names_of_a_nf. *)
