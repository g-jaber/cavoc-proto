module Make (IntLTS : Bipartite.LTS) = struct
  ...
  let rec generate ~show_moves ~get_move conf =
    match conf with
    | IntLTS.Active act_conf ->
        ...
        begin
          ...
          | (Some pas_conf, Vis output_move) ->
              ...
              generate ~show_moves ~get_move (IntLTS.Passive pas_conf)
        end
    | IntLTS.Passive pas_conf ->
        ...
        let string_list = List.map IntLTS.Actions.Moves.string_of_move moves_list in
        ...
        generate ~show_moves ~get_move (IntLTS.Active act_conf)
  ...
  let compute_graph  ~show_moves ~get_move act_conf =
    let result = generate ~show_moves ~get_move act_conf in
    get_trace result
end
