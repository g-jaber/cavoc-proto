...
(** What the user did when the interaction loop asked them to act.
    ...
    that consults the user returns a [user_action]. Quitting is thus part of the
...
module type IBUILD = sig
  ...
  (** [UserMonad] is the monad used to interact with the user. The backend does
      not impose any restriction on this monad, it is only here because it could
      be needed by some of the callbacks passed to {!val: interactive_build}. *)

  type conf

  type pol_move
  (** The moves the loop plays, reported to [record_move]. *)
  ...
  (** [interactive_build] starts the interaction loop. The environment is
      ...
      @param show_move
        display a textual representation of the move after each turn, together
        with the {!type:player} who made it.
      @param show_conf display the current configuration after each turn
      @param show_moves_list
        display the list of possibles moves when it's the user's turn.
      @param get_move
        returns the move choosen by the user, or {!constructor:Quit} if they
        chose to leave the game instead.
      @param record_move
        receives every move played, of either polarity, in chronological order:
        the play as it is being built.

      @return Why the interaction stopped *)
end

(** [RUN_LTS] is used by {!module: Make} to signify that it accepts any
    {{!module-type: Strategy.LTS}LTS}, provided that the LTS is capable of
    resolving non-determinism caused by an interpreter returning multiple
    ...
    monad {!module: RUN_LTS.UserMonad} is required. For the same reason [choose]
    returns a {!type: user_action}: asking the user which configuration to keep
    is another point at which they may leave the game.

    See {!val: Strategy.LTS.p_trans} See {!module: IBUILD.UserMonad} *)
...
module Make : functor
  ...
  ->
  IBUILD
    with module UserMonad = UserMonad
     and type conf = IntLTS.conf
     and type pol_move = IntLTS.TypingLTS.Moves.pol_move
