...
functor
  ...
  struct
    ...
    let string_of_graph { states; edges } =
      ...
      let edges_string =
        String.concat ", " (List.map string_of_transition edges) in
      "{" ^ states_string ^ "}\n" ^ "{" ^ edges_string ^ "}\n"
    ...
    let rec compute_graph_monad act_conf : unit m =
      ...
      Util.Debug.print_debug
        ("Adding the active state: " ^ string_of_state act_state);
      ...
      | Some pas_conf ->
          ...
          Util.Debug.print_debug
            ("Adding the passive configuration: " ^ string_of_state pas_state);
          ...
          begin
            ...
            | None ->
                ...
                let edge =
                  PublicTrans
                    ( pas_state,
                      IntLTS.Actions.inject_move input_move,
                      act_state' ) in
                let* () = add_edge edge in
