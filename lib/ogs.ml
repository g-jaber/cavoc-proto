module OgsLtsF = functor (M:Util.Monad.BRANCH) (Int:Interactive.INT) -> struct
  ...
  let p_trans act_conf =
    let opconf_option = Int.OpLang.compute_nf (act_conf.computation,act_conf.heap) in
    match opconf_option with
      ...
      | Some ((_,heap) as opconf) -> 
        ...
        let (move,ienv',namectxP') = Int.generate_output_action act_conf.namectxO nn value in
        (move,Some {heap; 
  ...
  let o_trans_gen pas_conf  =
    let* (input_move,lnamectx) = M.para_list (Int.generate_input_moves pas_conf.namectxP) in
    let computation = Int.trigger_computation pas_conf.ienv input_move in
    return (input_move,
