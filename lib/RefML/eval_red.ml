...
let rec red (expr, env, heap) =
  ...
  | App (Fix ((idfun, _), (var, ty), expr1), expr2) when isval expr2 ->
      ...
      ((expr', env', heap), true)
  | App _ -> failwith "Evaluation of terms not in ANF is not yet implemented."
  | Seq (Unit, expr2) -> ((expr2, Util.Pmap.empty, heap), true)
  | Seq (expr1, expr2) ->
      let ((expr1', env', heap'), isred) = red (expr1, env, heap) in
      ((Seq (expr1', expr2), env', heap'), isred)
  | While (Bool false, _) -> ((Unit, Util.Pmap.empty, heap), true)
  ...
  | LetPair (var1, var2, Pair (expr1, expr2), expr) -> begin
  ...
  | If (Bool b, expr1, expr2) ->
      if b then ((expr1, Util.Pmap.empty, heap), true)
  ...
  | UnaryOp (Not, Bool b) -> ((Bool (not b), Util.Pmap.empty, heap), true)
  | UnaryOp (Not, expr) ->
      let ((expr', env', heap'), isred) = red (expr, env, heap) in
