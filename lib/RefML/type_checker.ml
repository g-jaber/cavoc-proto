...
let rec infer_type vctx lctx nctx tsubst expr =
  match expr with
  | Var x -> begin
      match Util.Pmap.lookup x vctx with
  ...
  | Name n -> begin
      match Util.Pmap.lookup n nctx with
      | Some ty -> (ty, vctx, tsubst)
      | None ->
          Util.Error.fail_error
            ("Error: the name " ^ Syntax.string_of_name n
           ^ " is not defined in the environment "
            ^ Type_ctx.string_of_name_ctx nctx
            ^ " .")
    end
  | Loc l -> begin
      match Util.Pmap.lookup l lctx with
      | Some ty -> (ty, vctx, tsubst)
      | None ->
          Util.Error.fail_error
  ...
  | Named (cn, e) -> begin
      match Util.Pmap.lookup cn nctx with
  ...
  | BinaryOp (And, e1, e2) | BinaryOp (Or, e1, e2) ->
      check_type_bin vctx lctx nctx tsubst TBool e1 e2 TBool
  ...
  | If (e1, e2, e3) ->
      ...
      Util.Debug.print_debug
        ("Typing of the first branch:" ^ string_of_typeML ty2' ^ " (was "
       ^ string_of_typeML ty2 ^ ")");
      Util.Debug.print_debug
        ("Typing of the second branch:" ^ string_of_typeML ty3);
      begin
        match unify_type tsubst3 (ty2', ty3) with
        | Some (ty, tsubst4) ->
            Util.Debug.print_debug ("Typing If:" ^ string_of_typeML ty);
            (ty, Type_ctx.lsubst_vctx tsubst4 vctx3, tsubst4)
        | None ->
            Util.Error.fail_error
              ("Error typing "
              ^ Syntax.string_of_exprML expr
  ...
  | Fun ((var, TUndef), e) ->
      ...
      let (ty', vctx', tsubst') =
        infer_type (Util.Pmap.modadd_pmap (var, tvar) vctx) lctx nctx tsubst e
      in
      begin
        match Util.Pmap.lookup var vctx' with
        | Some ty -> (TArrow (ty, ty'), vctx', tsubst')
        | None ->
            failwith
              ("Variable " ^ Syntax.string_of_id var
             ^ " not found in type-checking. Please report.")
      end
  | Fun ((var, ty), e) ->
      let (ty', vctx', tsubst') =
        infer_type (Util.Pmap.modadd_pmap (var, ty) vctx) lctx nctx tsubst e
      in
  ...
  | Fix ((idfun, TUndef), (var, TUndef), e) ->
      ...
      let new_vctx =
        Util.Pmap.modadd_pmap2 (var, tvar1) (idfun, TArrow (tvar1, tvar2)) vctx
      in
      let (rty, vctx', tsubst') = infer_type new_vctx lctx nctx tsubst e in
      begin
        match (Util.Pmap.lookup var vctx', Util.Pmap.lookup idfun vctx') with
        | (Some aty, Some fty) -> begin
            match unify_type tsubst' (fty, TArrow (aty, rty)) with
            | Some (ty, tsubst'') ->
                (ty, Type_ctx.lsubst_vctx tsubst'' vctx', tsubst'')
            | None ->
                Util.Error.fail_error
                  ("Error typing "
                  ^ Syntax.string_of_exprML expr
  ...
  | Fix ((idfun, TUndef), (var, aty), e) ->
      let tvar2 = fresh_typevar () in
      let new_vctx =
        Util.Pmap.modadd_pmap2 (var, aty) (idfun, TArrow (aty, tvar2)) vctx
      in
      let (rty, vctx', tsubst') = infer_type new_vctx lctx nctx tsubst e in
      begin
        match Util.Pmap.lookup idfun vctx' with
        | Some fty -> begin
            match unify_type tsubst' (fty, TArrow (aty, rty)) with
            | Some (ty, tsubst'') ->
                (ty, Type_ctx.lsubst_vctx tsubst'' vctx', tsubst'')
  ...
  | LetPair (var1, var2, e1, e2) ->
      ...
      begin
        ...
        | TProd (ty1, ty2) ->
            let new_vctx =
              Util.Pmap.modadd_pmap2 (var1, ty1) (var2, ty2) vctx' in
            infer_type new_vctx lctx nctx tsubst' e2
        | TVar tvar ->
            let tvar1 = fresh_typevar () in
            let tvar2 = fresh_typevar () in
            let vctx'' = Type_ctx.subst_vctx tvar (TProd (tvar1, tvar2)) vctx' in
            let new_vctx =
              Util.Pmap.modadd_pmap2 (var1, tvar1) (var2, tvar2) vctx'' in
            let new_tsubst =
              Util.Pmap.modadd_pmap (tvar, TProd (tvar1, tvar2)) tsubst' in
  ...
  | App (e1, e2) ->
      ...
      begin
        ...
        | TVar a ->
            ...
            ( tvar,
              Type_ctx.subst_vctx a new_type_a vctx'',
              Util.Pmap.modadd_pmap (a, new_type_a) tsubst'' )
        | TArrow (ty', rty) -> begin
            match unify_type tsubst'' (ty', aty') with
            | Some (_, tsubst''') ->
                (rty, Type_ctx.lsubst_vctx tsubst''' vctx'', tsubst''')
            | None ->
                Util.Error.fail_error
                  ("Error typing "
                  ^ Syntax.string_of_exprML expr
  ...
  | Newref (_, e) ->
      ...
      begin
        ...
        | TVar a ->
            ( TRef TInt,
              Type_ctx.subst_vctx a TInt vctx',
              Util.Pmap.modadd_pmap (a, TInt) tsubst' )
  ...
  | Assign (e1, e2) ->
      ...
      begin
        ...
        | (TRef ty1', TVar a) ->
            (TUnit, Type_ctx.subst_vctx a ty1' vctx'', tsubst'')
        | (TRef (TVar a), _) ->
            (TUnit, Type_ctx.subst_vctx a ty2 vctx'', tsubst'')
        | (_, _) ->
            Util.Error.fail_error
              ("Error typing "
              ^ Syntax.string_of_exprML expr
...
and check_type vctx lctx nctx tsubst expr res_ty =
  ...
  | TVar a ->
      ( res_ty,
        Type_ctx.subst_vctx a res_ty vctx',
        Util.Pmap.modadd_pmap (a, res_ty) tsubst' )
...
and check_type_bin vctx lctx nctx tsubst com_ty expr1 expr2 res_ty =
  ...
  Util.Debug.print_debug
    ...
    ^ " with " ^ string_of_typeML com_ty ^ ":" ^ string_of_typeML ty1'
    ^ " and  " ^ string_of_typeML ty2);
  match (ty1', ty2) with
  | (TVar a1, TVar a2) ->
      let vctx_new = Type_ctx.subst_vctx a1 com_ty vctx'' in
      let vctx_new' = Type_ctx.subst_vctx a2 com_ty vctx_new in
      ( res_ty,
        vctx_new',
        Util.Pmap.modadd_pmap2 (a1, com_ty) (a2, com_ty) tsubst'' )
  | (TVar a1, _) when ty2 = com_ty ->
      let vctx_new = Type_ctx.subst_vctx a1 com_ty vctx'' in
      (res_ty, vctx_new, Util.Pmap.modadd_pmap (a1, com_ty) tsubst'')
  | (_, TVar a2) when ty1 = com_ty ->
      let vctx_new = Type_ctx.subst_vctx a2 com_ty vctx'' in
      (res_ty, vctx_new, Util.Pmap.modadd_pmap (a2, com_ty) tsubst'')
  | (_, _) when ty1' = com_ty && ty2 = com_ty -> (res_ty, vctx'', tsubst'')
  | (_, _) when ty1' <> com_ty ->
      raise
        (TypingError
           (string_of_typeML ty1 ^ " is not equal to " ^ string_of_typeML com_ty))
  | (_, _) ->
      raise
        (TypingError
           (string_of_typeML ty2 ^ " is not equal to " ^ string_of_typeML com_ty))
...
let typing_full tsubst expr =
  ...
  let name_ctx =
    ...
    @@ List.map
         ...
         lnames in
  let (ty, _, _) =
    infer_type Type_ctx.empty_var_ctx Type_ctx.empty_loc_ctx name_ctx tsubst
      expr in
  ty
