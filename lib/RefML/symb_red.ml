...
let add_locvar heap v = let l = fresh_locvar () in (l,Util.Pmap.add (l,v) heap)
...
let aux_bin_arith iop consfun expr1 expr2 heapPost symbred =
  match (expr1, expr2) with
    | (Int n1, Int n2) -> 
      let n = iop n1 n2 in 
      ([(Int n,Util.Pmap.empty,Util.Pmap.empty,heapPost,Util.Pmap.empty,[])],true)
    | (Int n, Var x) -> 
      let newid = fresh_lvar () in
      let newvar = Var newid in
      ([(newvar,Util.Pmap.empty,Util.Pmap.empty,heapPost,Util.Pmap.singleton(newid,Types.TInt),
...
let aux_bin_arithbool iop consfun expr1 expr2 heapPost symbred =
  match (expr1,expr2) with
    | (Int n1, Int n2) ->
      let b = iop n1 n2 in
      ([(Bool b,Util.Pmap.empty,Util.Pmap.empty,heapPost,Util.Pmap.empty,[])],true)
    | (Int n, Var x) ->
      ([(Bool true,Util.Pmap.empty,Util.Pmap.empty,heapPost,Util.Pmap.empty,[expr_to_arith_pred (consfun (Int n,Var x))])
        ;(Bool false,Util.Pmap.empty,Util.Pmap.empty,heapPost,Util.Pmap.empty,[negate_arith_pred (expr_to_arith_pred (consfun (Int n,Var x)))])]
       ,true)
    | (Var x, Int n) ->
      ([(Bool true,Util.Pmap.empty,Util.Pmap.empty,heapPost,Util.Pmap.empty,[expr_to_arith_pred (consfun (Var x,Int n))])
        ;(Bool false,Util.Pmap.empty,Util.Pmap.empty,heapPost,Util.Pmap.empty,[negate_arith_pred (expr_to_arith_pred (consfun (Var x,Int n)))])]
       ,true)
    | (Var x1, Var x2) ->
      ([(Bool true,Util.Pmap.empty,Util.Pmap.empty,heapPost,Util.Pmap.empty,[expr_to_arith_pred (consfun (Var x1,Var x2))])
        ;(Bool false,Util.Pmap.empty,Util.Pmap.empty,heapPost,Util.Pmap.empty,[negate_arith_pred (expr_to_arith_pred (consfun (Var x1,Var x2)))])]
       ,true)
    | (expr1,expr2) -> aux_bin_red symbred consfun (expr1,expr2)
...
let rec symbred heapPost expr = match expr with
  | App (Fun ((var,_),expr1),expr2) when (isval expr2) -> 
      ([(subst_var expr1 var expr2, Util.Pmap.empty, Util.Pmap.empty, heapPost, Util.Pmap.empty, [])],true)
  | App (Fix ((idfun,_),(var,ty),expr1),expr2) when (isval expr2) ->
      ([(subst_var expr1 var expr2, Util.Pmap.singleton (idfun,Fun ((var,ty),expr1)), Util.Pmap.empty, heapPost, Util.Pmap.empty, [])],true)
  | App (expr1,expr2) -> 
      aux_bin_red (symbred heapPost) (fun (x,y) -> App (x,y)) (expr1,expr2)
  | Seq (Unit,expr2) -> 
    ([(expr2,Util.Pmap.empty,Util.Pmap.empty,heapPost,Util.Pmap.empty, [])],true)
  | Seq (expr1,expr2) -> 
  ...
  | Let (var,expr1,expr2) when (isval expr1) -> 
      ([(subst_var expr2 var expr1, Util.Pmap.empty, Util.Pmap.empty, heapPost, Util.Pmap.empty, [])],true)
  ...
  | LetPair (var1,var2,Pair (expr1,expr2),expr') when (isval expr1 && isval expr2)->
      ...
      ([(expr'', Util.Pmap.empty, Util.Pmap.empty, heapPost, Util.Pmap.empty, [])],true)
  ...
  | Newref (ty,expr) -> 
    if (isval expr) then 
      ...
      ([(Var l,Util.Pmap.empty,Util.Pmap.empty,heapPost',Util.Pmap.singleton (l,Types.TRef Types.TInt), [])],true) (* Fix This *)
     else let (result,b) = symbred heapPost expr in 
      (List.map (aux (fun x -> (Newref (ty,x)))) result,b)
  | Deref (Var l) -> 
    begin match lookup l heapPost with
      | Some value -> ([(value,Util.Pmap.empty,Util.Pmap.empty,heapPost,Util.Pmap.empty, [])],true)
      | None -> let x = Logic.fresh_lvar () in
                let heapPre = Util.Pmap.singleton (l,Var x) in 
                ([(Var x,Util.Pmap.empty,heapPre,Util.Pmap.concat heapPre heapPost,Util.Pmap.singleton (x,Types.TInt), [])],true) (* Fix This *)
    end
  | Deref expr -> let (result,b) = symbred heapPost expr in (List.map (aux (fun x -> (Deref x))) result,b)
  | Assign (Var l,expr2) when (isval expr2) ->
    begin match lookup l heapPost with
      | Some _ ->  ([(Unit,Util.Pmap.empty,Util.Pmap.empty,modadd_pmap (l,expr2) heapPost,Util.Pmap.empty, [])],true)
      | None -> let x = Logic.fresh_lvar () in
                let heapPre = Util.Pmap.singleton (l,Var x) in 
                ([(Unit,Util.Pmap.empty,heapPre,modadd_pmap (l,expr2) heapPost,Util.Pmap.singleton (x,Types.TInt), [])],true)
    end
  | Assign (expr1,expr2) ->  aux_bin_red (symbred heapPost) (fun (x,y) -> Assign (x,y)) (expr1,expr2)
  | If (Bool b,expr1,expr2) -> 
      if b then 
        ([(expr1,Util.Pmap.empty,Util.Pmap.empty,heapPost,Util.Pmap.empty, [])],true) 
      else ([(expr2,Util.Pmap.empty,Util.Pmap.empty,heapPost,Util.Pmap.empty, [])],true)
  | If ((Var _),_,_) -> 
  ...
  | UnaryOp(Not,Bool b) -> 
      ([(Bool (not b),Util.Pmap.empty,Util.Pmap.empty,heapPost,Util.Pmap.empty, [])],true)
(*  | Not (Var b) -> [(Bool true, [],[],heapPost,[AEqual (AExpr(Var b)),(Bool false)]);(Bool false, [], [], heapPost,[AEqual (AExpr(Var b)),(Bool true)])]*)
  ...
  | _ -> ([(expr,Util.Pmap.empty,Util.Pmap.empty,heapPost,Util.Pmap.empty, [])],false)


let rec symbred_trans (expr,gamma,heapPre,heapPost,vars,preds) =
  let aux (expr',gamma',heapPre',heapPost',vars',preds') = 
    (expr',Util.Pmap.concat gamma' gamma,Util.Pmap.concat heapPre' heapPre,heapPost',Util.Pmap.concat vars' vars,preds'@preds) in
  let (result,b) = symbred heapPost expr in
  if (not b) then List.map aux result
  else List.flatten (List.map
         (fun (expr',gamma',heapPre',heapPost',vars',preds') ->
            symbred_trans (expr',Util.Pmap.concat gamma' gamma,Util.Pmap.concat heapPre' heapPre,heapPost',
              Util.Pmap.concat vars' vars,preds'@preds)) result)

let compute_nf expr =
  symbred_trans (expr,Util.Pmap.empty,Util.Pmap.empty,Util.Pmap.empty,Util.Pmap.empty,[])