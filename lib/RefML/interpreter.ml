...
module SymbolicEvalState = struct
  ...
  let alloc expr : loc m = fun store -> [ Store.loc_allocate store expr ]
  ...
  let get loc : value m =
   fun store ->
    ...
    | None ->
        failwith
          (string_of_loc loc ^ " is not in the store "
          ^ Store.string_of_store store)

  let set loc expr : unit m =
   fun store -> [ ((), Store.loc_modify store loc expr) ]

  let store () = fun store -> [ (store, store) ]
  let set_store store = fun _ -> [ ((), store) ]
  let para_pair ma mb = fun store -> ma store @ mb store
  let run m store = m store
  let fail () = fun _ -> []
  ...
  let nondet = function
    ...
    | ty ->
        failwith
          ("symbolic variables of type " ^ Types.string_of_typ ty
         ^ " not supported")

  (** select computation t and/or f depending on the satisfiability of k/not k.
  *)
...
let interpreter interpreter expr : value SymbolicEvalState.m =
  ...
  | Constructor (_cons, None) ->
      failwith "Empty constructor not implemented yet (interpreter)"
  | App (expr1, expr2) ->
  ...
  | Seq (expr1, expr2) ->
      let* expr1 = interpreter expr1 in
      if expr1 = Unit then interpreter expr2 else return @@ Seq (expr1, expr2)
  | While (guard, body) ->
      let* guard = interpreter guard in
      begin
        let loop () =
          let* body = interpreter body in
          if body = Unit then interpreter expr
          else return @@ Seq (body, While (guard, body)) in
        match guard with
  ...
  | Let (var, expr1, expr2) ->
      ...
      if isval expr1 then
        ...
        interpreter expr
      else return @@ Let (var, expr1, expr2)
  ...
  | Newref (ty, expr) ->
      ...
      if isval nf then
        let* l = alloc nf in
        return @@ Loc l
      else return @@ Newref (ty, nf)
  | Deref expr ->
      let* nf = interpreter expr in
      begin match nf with Loc l -> get l | _ -> return @@ Deref nf
      end
  | Assign (expr1, expr2) ->
      let* nf1 = interpreter expr1 in
      begin match nf1 with
  ...
  | BinaryOp ((And as op), expr1, expr2) | BinaryOp ((Or as op), expr1, expr2)
    ->
      let* nf1 = interpreter expr1 in
      begin match (nf1, op) with
      | (Bool true, Or) | (Bool false, And) ->
          return @@ Bool (op = Or) (* short circuit *)
      | (Bool b1, _) ->
          let* nf2 = interpreter expr2 in
          begin match nf2 with
          | Bool b2 -> return @@ Bool (Syntax.implement_bin_bool_op op b1 b2)
          | Symbolic k2 -> return @@ Symbolic k2
          | _ -> return @@ BinaryOp (op, nf1, nf2)
          end
      | (Symbolic k1, _) ->
          let* nf2 = interpreter expr2 in
          begin match (nf2, op) with
          | (Bool true, Or) | (Bool false, And) -> return @@ Bool (op = Or)
          | (Bool _, _) -> return @@ Symbolic k1
          | (Symbolic k2, _) ->
  ...
  | UnaryOp (Not, expr) ->
      let* nf = interpreter expr in
      begin match nf with
      | Symbolic k1 -> return @@ Symbolic (Symbolic.neg k1)
  ...
  | TryWith (expr, handler_l) ->
      let* nf = interpreter expr in
      if isval nf then return nf
      else begin
        match nf with
        | Raise (Constructor _ as cons) ->
            let rec aux = function
              | Handler (pat, expr_pat) :: rest -> begin
                  match match_pattern_with_value pat cons with
                  | Some substitution ->
                      let substitute e (id, v) = subst_var e id v in
                      interpreter (List.fold_left substitute expr_pat substitution)
                  | None -> aux rest
                end
              | [] -> return @@ Raise cons in
            aux handler_l
  ...
  | Match (expr, handler_l) ->
      ...
      if isval nf then
        let rec select_branch = function
          | Handler (pat, expr_branch) :: rest -> begin
              match match_pattern_with_value pat nf with
              | Some substitution ->
                  let substitute e (id, v) = subst_var e id v in
                  interpreter (List.fold_left substitute expr_branch substitution)
              | None -> select_branch rest
            end
          | [] -> return Error in
        select_branch handler_l
      else return @@ Match (nf, handler_l)
  | Record fields ->
      let reconstruct_fields m (id, expr) =
        let* new_fields = m in
        let* expr' = interpreter expr in
        return @@ Util.Pmap.add (id, expr') new_fields in
...
let normalize_opconf (value, store) = run (normalize_opconf_fix value) store
  ...
  let rec aux store = function
    ...
    | (var, comp) :: comp_list' ->
        if isval comp then
          ...
          | _ ->
              failwith
                "Error: Non determinism in the initial evaluation. Please \
                 report."
        end in
  let store =
