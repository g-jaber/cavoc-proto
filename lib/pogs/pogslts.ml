(* The Opponent chooses the value of every public location when it plays,
   and the Player answers with its whole ground heap made public. *)
module Make (Lang : Lang.Interactive.LANG_WITH_INIT) :
  ...
  (* Levels go first to the locations the move mentions, then to the ground
     ones of the heap. *)
  let p_trans (act_conf : active_conf) =
    let open EvalMonad in
    let namectxO = Lang.IEnv.im act_conf.ienv in
    let* ((a_nf, _, _), ienv, store) =
      Lang.eval (act_conf.opconf, namectxO, TypingLTS.get_storectx act_conf.pos)
    in
    let store = Lang.disclose_heap store in
    let (a_nf, ienv) = Lang.complete_abstract_store store (a_nf, ienv) in
    let move = (TypingLTS.Moves.Output, (a_nf, Lang.IEnv.dom ienv)) in
    let (_, pos) = TypingLTS.trigger_move act_conf.pos move in
    let ienv = Lang.IEnv.copairing act_conf.ienv ienv in
    return (move, ({ store; ienv; pos } : passive_conf))
  ...
  let o_trans (pas_conf : passive_conf) ((_, (a_nf, _)) as input_move) =
    ...
    | Some (weakening, pos) ->
        ...
        Some ({ opconf; ienv; pos } : active_conf)
  ...
  let o_trans_gen (pas_conf : passive_conf) =
    ...
    let (opconf, ienv) = concretize pas_conf a_nf weakening in
    return (input_move, ({ opconf; ienv; pos } : active_conf))
  ...
  let init_pconf store ienv namectxP namectxO =
    let store = Lang.disclose_heap store in
    let pos =
      TypingLTS.init_pas_pos (Lang.infer_type_store store) namectxP namectxO in
    { store; ienv; pos }
