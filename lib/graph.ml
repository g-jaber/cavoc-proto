...
module Graph : GRAPH = functor (IntLTS : Bilts.INT_LTS) -> struct
  ...
  let string_of_state = function
    | ActState (aconf,id) -> (IntLTS.string_of_active_conf aconf) ^ "_" ^ (string_of_id_state id)
    | PasState (pconf,id) -> (IntLTS.string_of_passive_conf pconf) ^ "_" ^ (string_of_id_state id)
  ...
  | PublicTrans (st1,act,st2) ->
    (idstring_of_state st1) ^ " -"^ (IntLTS.Int.Actions.string_of_action act) ^"-> " ^ (idstring_of_state st2)
