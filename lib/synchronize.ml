...
let rec synchronize_check confP confO =
  ...
  match (pas_conf_option,IntLTS.Actions.get_move_from_action action) with
    ...
    | (Some pas_conf',Some output_move) ->
      ...
      | None ->
        Util.Debug.print_debug ("Input move forbidden: " ^ (IntLTS.Actions.Moves.string_of_move input_move)
      ...
      | Some act_conf' -> 
        ...
        Util.Debug.print_debug @@ "Synching succeeded with move " ^ (IntLTS.Actions.Moves.string_of_move moveP); 
...
let rec synchronize_gen nspan confP confO =
  ...
  match (pas_conf_option,IntLTS.Actions.get_move_from_action action) with
    ...
    | (Some pas_conf',Some output_move) ->
      ...
      Util.Debug.print_debug ("Synching moves " ^ (IntLTS.Actions.Moves.string_of_move moveP) ^" and " ^ (IntLTS.Actions.Moves.string_of_move moveO));
      let nspan_option = IntLTS.Int.synch_move nspan moveP moveO in
      begin match nspan_option with
        | None -> 
          let span_string = Util.Namespan.string_of_span IntLTS.Int.OpLang.string_of_name nspan in
