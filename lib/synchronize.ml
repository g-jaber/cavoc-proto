...
include Monad.LWMonad(struct type t = (IntLTS.Int.Actions.action) end)

let rec synchronize nspan confP confO =
  Debug.print_debug "One step of synchronization";
  ...
  match pas_conf_option with
    | None -> Debug.print_debug "Stopping synchronization"; print act_move
    | Some pas_conf' ->
      ...
      Debug.print_debug ("Synching moves " ^ (IntLTS.Int.Actions.string_of_action pmove) ^" and " ^ (IntLTS.Int.Actions.string_of_action omove));
      let nspan_option = IntLTS.Int.Actions.synch_action nspan pmove omove in
      begin match nspan_option with
        | None -> Debug.print_debug ("Synching failed in namespan " ^ (Pmap.string_of_pmap "" "," IntLTS.Int.Actions.Lang.string_of_name IntLTS.Int.Actions.Lang.string_of_name) nspan);
          return ()
        | Some nspan' -> Debug.print_debug "Synching succeeded"; let* () = print pmove in (synchronize nspan' confP' confO')
      end
