module Make
    ...
    (HistLts : Hislts.HISLTS_INIT
                 with type move = IntLts.Int.Actions.Moves.move
                  and type name = IntLts.Int.OpLang.name) :
  Bilts.INT_LTS with module Int = IntLts.Int = struct
  module M = IntLts.M
  open M
  ...
  let init_pconf resources ienv namectxP namectxO =
    let init_iconf = IntLts.init_pconf resources ienv namectxP namectxO in
    let init_hconf =
      HistLts.init_pconf (Util.Pmap.dom namectxP) (Util.Pmap.dom namectxO) in
    (init_iconf, init_hconf)
end
