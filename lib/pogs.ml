module PogsLtsF = functor (M:Util.Monad.BRANCH) (Int:Interactive.INT)  -> struct
  ...
  let p_trans aconf =
    ...
    match opconf_option with
      ...
      | Some ((_,heap) as opconf) -> 
        ...
        let (move,ienv,namectxP) = Int.generate_output_action aconf.namectxO nn value in
  ...
  let o_trans_gen pconf  =
    let* (input_move,lnamectx) = M.para_list (Int.generate_input_moves pconf.namectxP) in
