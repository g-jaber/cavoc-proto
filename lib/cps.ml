module Moves_Make (CpsLang:Lang.Cps.LANG) = struct
  ...
  let string_of_move (direction,nn,nup) = (CpsLang.string_of_name nn) ^ (string_of_direction direction) ^ "(" ^ (CpsLang.string_of_nup nup) ^ ")"
  ...
  let get_transmitted_names (_,_,nup) = 
    CpsLang.names_of_nup nup
  let get_subject_names (_,nn,_)= [nn]
end

module type INT = sig
  module ContNames : Lang.Cps.CONT_NAMES
  include Interactive.INT
end

module Int_Make (CpsLang : Lang.Cps.LANG) :
  INT with type OpLang.name = CpsLang.name and type ContNames.name = CpsLang.name and type Actions.Moves.name = CpsLang.name = struct
  ...
  let check_input_move namectxP (dir,name,nup) =
   match dir with
    ...
    let trigger_computation ienv input_move = match input_move with
      | (Moves.Input,name,nup) -> 
        begin match CpsLang.lookup_ienv name ienv with
        ...
        | None -> 
          failwith ("Error: the move " ^ (Moves.string_of_move input_move) ^ " is ill-formed: the name "
            ^ (CpsLang.string_of_name name) ^ " is not in the environment " ^ (CpsLang.string_of_ienv ienv) 
            ^ ". Please report.")
        end
    ...
    let unify_move span move1 move2 =
      match (move1,move2) with
        ...
        | ((Moves.Input,nn1,nup1),(Moves.Input,nn2,nup2)) -> 
          ...
          then CpsLang.unify_nup span nup1 nup2
          else None
        | _ -> None 
