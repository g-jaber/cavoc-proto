...
let rec infer_type vctx lctx nctx tsubst expr = match expr with
  | Var x ->
    begin match Pmap.lookup x vctx with
      | Some ty -> (ty,vctx,tsubst)
      | None -> Error.fail_error ("Error: the variable " ^ (Syntax.string_of_id x) ^ " is not defined in the environment "
          ^ (string_of_var_ctx vctx) ^
          " .")
    end
  | Name n ->
    begin match Pmap.lookup n nctx with
    | Some ty -> (ty,vctx,tsubst)
    | None -> Error.fail_error ("Error: the name " ^ (Syntax.string_of_name n) ^ " is not defined in the environment "
        ^ (string_of_name_ctx nctx) ^
        " .")
  end
  | Loc l ->
    begin match Pmap.lookup l lctx with
      | Some ty -> (ty,vctx,tsubst)
      | None -> Error.fail_error ("Error: the location " ^ (Syntax.string_of_loc l) ^ " is not defined.")
    end
  | Named (cn,e) ->
    begin match Pmap.lookup cn nctx with
      | Some (TNeg ty) -> check_type vctx lctx nctx tsubst e ty
      | Some ty -> Error.fail_error ("Error: the continuation name " ^ (Syntax.string_of_name cn) ^ " is of type "
                                     ^ string_of_typeML ty ^".")      
      | None -> Error.fail_error ("Error: the continuation name " ^ (Syntax.string_of_name cn) ^ " is not defined.")
    end
  ...
  | Fun ((var,TUndef),e) ->
    ...
    let (ty',vctx',tsubst') = infer_type (Pmap.modadd_pmap (var,tvar) vctx) lctx nctx tsubst  e in
    begin match Pmap.lookup var vctx' with
      | Some ty -> (TArrow (ty,ty'),vctx',tsubst')
      | None -> failwith ("Variable " ^ (Syntax.string_of_id var) ^ " not found in type-checking. Please report.")
    end
  | Fun ((var,ty),e) ->
    let (ty',vctx',tsubst') = infer_type (Pmap.modadd_pmap (var,ty) vctx) lctx nctx tsubst e in
  ...
  | Fix ((idfun,TUndef),(var,TUndef),e) ->
    ...
    begin match (Pmap.lookup var vctx',Pmap.lookup idfun vctx') with
      | (Some aty, Some fty) ->
  ...
  | Fix ((idfun,TUndef),(var,aty),e) ->
    ...
    let (rty,vctx',tsubst') = infer_type new_vctx lctx nctx tsubst e in
    begin match Pmap.lookup idfun vctx' with
      | Some fty -> 
        begin match unify_type tsubst' (fty,TArrow (aty,rty)) with
          | Some (ty,tsubst'') -> (ty,lsubst_vctx tsubst'' vctx',tsubst'')
          | None -> Error.fail_error ("Error typing " ^(Syntax.string_of_exprML expr) ^ ": "
