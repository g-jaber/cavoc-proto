...
let get_exprML nbprog filename =
  let inBuffer = open_in filename in
  let lineBuffer = Lexing.from_channel inBuffer in
  try let expr = Cavoc.Parser.fullexpr Cavoc.Lexer.token lineBuffer in
    Cavoc.Debug.print_debug (nbprog ^ " program: " ^ Cavoc.Syntax.string_of_exprML expr);
    let ty = Cavoc.Type_checker.typing_full expr in
    Cavoc.Debug.print_debug ("His type is: " ^ Cavoc.Types.string_of_typeML ty);
    (expr,ty)
  with
...
let get_prog nbprog filename =
  ...
  try let prog = Cavoc.Parser.prog Cavoc.Lexer.token lineBuffer in
    Cavoc.Debug.print_debug (nbprog ^ " program: " ^ Cavoc.Declaration.string_of_prog prog);
(*    let ty = Cavoc.Type_checker.typing_full in
    Cavoc.Debug.print_debug ("His type is: " ^ Cavoc.Types.string_of_typeML ty);
    (expr,ty)*)
  with
  | Cavoc.Lexer.SyntaxError msg -> failwith ("Parsing Error in the " ^ nbprog
...
let () =
  ...
  let _ = get_exprML "first" !filename
in ();;