...
let () =
  ...
  let (ienv, resources, namectxO') = Int.OpLang.get_typed_ienv inBuffer2 inBuffer3 in
  ...
  let init_pconf =
    ProdLTS.Passive (ProdLTS.init_pconf resources ienv namectxO' Util.Pmap.empty) in
